...
from D3.v2.field_calcs import compute_elastic_metrics
...
class ThermoelasticTopologyOptimization3D:
    def __init__(self, nelx, nely, nelz, volfrac, penal, rmin, lx=1.0, ly=1.0, lz=1.0, iter_solve=False, fname='design.npz', el_weight=0.5, plot=False, boundary_conditions=None, static_vf_init=None):
        self.nelx = nelx
        self.nely = nely
        self.nelz = nelz
        ...
        self.k0 = 1.0       # Thermal Conductivity (0.22 J/(smK))
        self.kmin = 1e-2  # was 1e-9
        ...
        self.iter = 0
        if static_vf_init is None:
            self.x = self.volfrac * np.ones(self.num_elems)
        else:
            self.x = static_vf_init * np.ones(self.num_elems)
    ...
    def _build_boundary_conditions(self):
        ...
        # Middle Face Nodes
        node_MF_TL =     (nodes_per_slice * (self.nelz//2)) + node_TF_TL
        node_MF_ML =     (nodes_per_slice * (self.nelz//2)) + node_TF_ML
        node_MF_BL =     (nodes_per_slice * (self.nelz//2)) + node_TF_BL
        node_MF_CENTER = (nodes_per_slice * (self.nelz//2)) + node_TF_CENTER
        node_MF_TR =     (nodes_per_slice * (self.nelz//2)) + node_TF_TR
        node_MF_BR =     (nodes_per_slice * (self.nelz//2)) + node_TF_BR
        nodes_MF = np.arange(node_MF_TL, node_MF_BR + 1)

        # Bottom Face Nodes
        node_BF_TL =     (nodes_per_slice * self.nelz) + node_TF_TL
        node_BF_ML =     (nodes_per_slice * self.nelz) + node_TF_ML
        node_BF_BL =     (nodes_per_slice * self.nelz) + node_TF_BL
        node_BF_CENTER = (nodes_per_slice * self.nelz) + node_TF_CENTER
        node_BF_TR =     (nodes_per_slice * self.nelz) + node_TF_TR
        node_BF_BR =     (nodes_per_slice * self.nelz) + node_TF_BR
        ...
        nodes_LF = (z_range[:, None] * nodes_per_slice + y_range[None, :]).flatten()

        # Right Face (x = nelx)
        # This is just the Left Face offset by the width of the domain (nelx * ny)
        nodes_RF = nodes_LF + (self.nelx * ny)
        ...
        nodes_BackF = (z_range[:, None] * nodes_per_slice + x_range[None, :] * ny).flatten()

        # Front Face (y = 0)
        # Corresponds to the "Top Edge" of every z-slice (TF_TL to TF_TR)
        # Formula: Index = (z * nodes_per_slice) + (x * ny) + 0
        nodes_FF = nodes_BackF + self.nely

        # # Try to sample a random 2x2 patch on the back face
        ...
        # ])
        #
        # # To move the patch nodes to the right face, add the offset
        # patch_nodes = patch_nodes + (self.nelx * ny)
        ...
        # ])
        #
        # # try offsetting to front face
        # patch_nodes = patch_nodes + self.nely

        # # Try to sample a 2x2 patch on the top face
        # start_x = np.random.randint(0, self.nelx)
        # start_y = np.random.randint(0, self.nely)
        ...
        # Now a 3x3 patch on the top face
        start_x = np.random.randint(0, self.nelx-1)
        start_y = np.random.randint(0, self.nely-1)
        start_node_idx = (start_y) + (start_x * (self.nely + 1))  # z=0
        patch_nodes = np.array([
        ...
        # Now offset to get the bottom face samples
        offset = nodes_per_slice * self.nelz
        patch_nodes = patch_nodes + offset
    ...
    def sensitivity_analysis(self, x, U1, T1, F_mech_ext, Ft, K_mech, K_therm, C_coup):
        ...
        # Only consider thermal coupling term if thermal weight > 0
        if self.th_weight > 0.0:
            dc = (self.el_weight * (term1_mech + term3_coup)) + (self.th_weight * term2_therm)
        else:
            dc = (self.el_weight * term1_mech) + (self.th_weight * term2_therm)
    ...
    def optimize(self, max_iter=100):
        ...
        records = {
            # Initial Conditions
            'bcs': deepcopy(self._build_boundary_conditions()),
            'seed_design': np.reshape(self.x, (self.nelz, self.nelx, self.nely)).copy().astype(np.float16),
            'strain_energy_field': None,
            'von_mises_stress_field': None,
        ...
        for k in range(max_iter):
            ...
            if k == 0:
                strain_energy, von_mises = compute_elastic_metrics(self, self.x, U1)
                records['strain_energy_field'] = np.reshape(strain_energy, (self.nelz, self.nelx, self.nely)).astype(np.float16)
                records['von_mises_stress_field'] = np.reshape(von_mises, (self.nelz, self.nelx, self.nely)).astype(np.float16)
                records['temperature_field'] = np.reshape(T1, (self.nelz+1, self.nelx+1, self.nely+1)).astype(np.float16)
                # self.plot_fields(strain_energy, von_mises, T1)

            # 2. Sensitivity
            dfdx, f_val, f_val_mech, f_val_therm = self.sensitivity_analysis(self.x, U1, T1, F_mech_ext, Ft, K_mech, K_therm, C_coup)
            records['opti_steps'].append({'structural_compliance': f_val_mech, 'thermal_compliance': f_val_therm})
            ...
            # Record updates
            records['design_steps'].append(np.reshape(self.x, (self.nelz, self.nelx, self.nely)).astype(np.float16))
            records['iterations'] += 1
            ...
            curr_vol_frac = current_vol / self.num_elems
            if np.abs(curr_vol_frac - self.volfrac) > 0.4 or curr_vol_frac in [0.0, 1.0]:
                print("Volume fraction deviated too much! Stopping optimization.")
                records['failed'] = True
                break
        ...
        records['design'] = np.reshape(self.x, (self.nelz, self.nelx, self.nely)).copy().astype(np.float16)
        records['structural_compliance'] = f_val_mech
        records['thermal_compliance'] = f_val_therm
        records['volume_fraction'] = current_vol / self.num_elems

        bcs = self._build_boundary_conditions()
        if 'records_path' in bcs:
            # save records to the file path as pickle
            with open(bcs['records_path'], 'wb') as f:
                pickle.dump(records, f)
            print(f"Saved optimization records to {bcs['records_path']}")
    ...
    def plot_fields(self, strain_energy, von_mises, temperature):
        design = np.reshape(self.x, (self.nelz, self.nelx, self.nely))

        strain_energy = np.reshape(strain_energy, (self.nelz, self.nelx, self.nely))
        von_mises = np.reshape(von_mises, (self.nelz, self.nelx, self.nely))
        temperature = np.reshape(temperature, (self.nelz + 1, self.nelx + 1, self.nely + 1))
        ...
        viewer.add_image(fixed_therm_nodes, name='fixed_thermal', rendering='attenuated_mip', visible=False, colormap='cyan')
        viewer.add_image(heat_gen_nodes, name='heat_generation', rendering='attenuated_mip', visible=False, colormap='yellow')

        viewer.add_image(strain_energy, name='strain_energy', rendering='mip', visible=False, colormap='blue')
        viewer.add_image(von_mises, name='von_mises', rendering='mip', visible=False, colormap='green')
        viewer.add_image(temperature, name='temperature', rendering='mip', visible=False, colormap='magma')
