...
class ThermalFlexureTopologyOptimization:
    def __init__(self, nelx, nely, volfrac, penal, rmin,
        ...
        self.Lx = phys_length_x                  # Meters
        self.Ly = phys_length_x * (nely / nelx)  # Meters (assume square elements)
        self.thickness = thickness               # Meters

        # Element Size (h)
        self.h = self.Lx / self.nelx
        self.elem_area = self.h ** 2
        self.elem_vol = self.elem_area * self.thickness

        # 3. Material Properties (From Data Class)
        self.mat = material
        self.E0 = self.mat.E           # Young's Modulus
        # self.Emin = self.mat.E * 1e-6  # Void stiffness
        self.Emin = 0.1  # Void stiffness
        self.nu = self.mat.nu          # Poisson ratio
        self.k0 = self.mat.k           # Thermal Conductivity (J/(smK))
        self.kmin = self.mat.k * 1e-6
        # self.kmin = 0.001
        self.alpha = self.mat.alpha    # Expansion coeff (K)
        self.rho0 = self.mat.rho       # Density (kg/m^3)
        self.Cp = self.mat.Cp          # Heat Capacity (J/(kgK))
        self.Cmin = self.mat.rho * self.mat.Cp * 1e-6

        # 4. Matrix Assembly with SCALING
        # Base Unit Matrices (Computed for 1x1x1 cube)
        self.Ke0_unit, self.Kt0_unit, self.Cethm0_unit = fe_melthm(
            self.mat.nu, self.mat.E, self.mat.k, self.mat.alpha
        )

        # --- SCALING RULES ---
        # Stiffness: Scales with thickness (independent of h for 2D square)
        self.Ke0 = self.Ke0_unit * self.thickness

        # Conductivity: Scales with thickness (independent of h)
        self.Kt0 = self.Kt0_unit * self.thickness

        # Thermal Force Coupling: Scales with Face Area (h * thickness)
        # The unit matrix calculated (E * alpha) is stress/temp.
        # Force = Stress * Area. Area = h * thickness.
        self.Cethm0 = self.Cethm0_unit * (self.h * self.thickness)

        # Heat Capacity: Scales with Volume (Area * thickness)
        # Note: Code uses simplified lumped mass logic
        total_heat_cap = self.elem_vol * self.rho0 * self.Cp
        self.Ce0 = (total_heat_cap / 4.0) * np.eye(4)

        # ... (rest of the INIT function: DOFs, MMA setup) ...
        ...
        self.a_mma = np.zeros(1)
        self.c_mma = 10000.0 * np.ones(1)
        self.d_mma = np.zeros(1)

        # 5. Determine Automatic Time Step (Characteristic Time)
        # Diffusivity D = k / (rho * Cp)
        # Time constant tau = L^2 / D
        diffusivity = self.mat.k / (self.mat.rho * self.mat.Cp)
        tau = (self.Lx ** 2) / diffusivity
        print(f"Material Diffusivity: {diffusivity:.2e} m^2/s")
        print(f"Characteristic Thermal Time (tau): {tau:.4f} s")

        # # Set tf based on tau (e.g., 20% of characteristic time to see transient effects)
        # self.tf = tau * 0.2
        # self.num_time_steps = 20
        # self.dt = self.tf / self.num_time_steps
        # print(f"Timestep size: {self.tf:.4f} s")
    ...
    def diagnose_magnitudes(self, x):
        print("--- DIAGNOSTIC START ---")
        K_mech, K_therm, C_mat = self._assemble_system(x)

        # 1. Check Stiffness Magnitude (Should be approx E * t ~ 1e5 to 1e7)
        print(f"Avg Stiffness (K_mech) diagonal: {np.mean(K_mech.diagonal()):.2e}")

        # 2. Check Capacity Magnitude (Should be approx rho * Cp * Vol ~ 1e-4 to 1e-6)
        # If this is 1e6, you forgot the volume scaling!
        print(f"Avg Capacity (C_mat) diagonal:   {np.mean(C_mat.diagonal()):.2e}")

        # 3. Check Conductivity Magnitude (Should be approx k * t ~ 1e-2)
        print(f"Avg Conductivity (K_therm) diag: {np.mean(K_therm.diagonal()):.2e}")

        # 4. Solve one time step manually to see Temp Rise
        Pt = np.zeros(self.ndof_therm)
        Pt[int(self.ndof_therm / 2)] = 100.0  # 100 Watts

        # Simple Euler step: C * dT/dt = P  ->  dT = P * dt / C
        # Using your dt
        dt = self.dt
        avg_C = np.mean(C_mat.diagonal())
        expected_dT = 100.0 * dt / avg_C

        print(f"Time Step (dt): {dt:.2e} s")
        print(f"Input Power: 100 W")
        print(f"Estimated Temp Rise per step: {expected_dT:.2e} K")

        if expected_dT < 1e-6:
            print(">> ERROR: Temperature rise is negligible. Check C_mat scaling.")
        else:
            print(">> OK: Temperature rise looks physically reasonable.")
    ...
    def _build_edof_matrices(self):
        ...
        for elx in range(self.nelx):
            for ely in range(self.nely):
                ...
                # n2 = (ely + 1) + elx * (self.nely + 1)        # Bottom Left
                # n3 = (ely + 1) + (elx + 1) * (self.nely + 1)  # Bottom Right
                # n4 = ely + (elx + 1) * (self.nely + 1)        # Top Right
    ...
    def solve_physics(self, x):
        ...
        # Example: Left Wall = 0 deg, Bottom Right patch = 100 deg

        # Nodes on Left Wall
        nodes_left = np.array([ely + 0 * (self.nely + 1) for ely in range(self.nely + 1)])

        # Nodes on Bottom Right (Heat Source Location)
        node_BR = self.nely + ((self.nely + 1) * self.nelx)
        # nodes_input = np.arange((self.nely + 1) * self.nelx, node_BR + 1)

        # Combine Fixed Nodes
        # fixed_therm_nodes = np.unique(np.concatenate((nodes_left, nodes_input)))
        fixed_therm_nodes = [node_BR]

        # Define Fixed Values
        fixed_therm_values = np.zeros(self.ndof_therm)
        fixed_therm_values[fixed_therm_nodes] = 100.0  # Set Heat Input Temp
        # fixed_therm_values[nodes_left] = 0.0  # Set Heat Sink Temp
    ...
    def solve_physics_transient(self, x):
        ...
        # Nodes on Right Wall (Heat Source Location)
        node_BR = self.nely + ((self.nely + 1) * self.nelx)
        nodes_input = np.arange((self.nely + 1) * self.nelx, node_BR + 1)

        # Combine Fixed Nodes
        # fixed_therm_nodes = np.unique(np.concatenate((nodes_left, nodes_input)))
        fixed_therm_nodes = nodes_input

        # Define Fixed Values
        fixed_therm_values = np.zeros(self.ndof_therm)
        fixed_therm_values[nodes_input] = 100.0  # Set Heat Input Temp
        # fixed_therm_values[nodes_left] = 0.0  # Set Heat Sink Temp
        ...
        for idx, Phi_final in enumerate(Phi_history):
            ...
            node_BL = self.nely  # x=0, y=nely
            # node_BM = self.nely + int(self.nelx / 2) * (self.nely + 1)  # x=half, y=nely
            node_TL = 0  # int(self.nely / 2)
            fixed_nodes = np.array([node_BL, node_TL])
            fixed_dofs = np.concatenate([2 * fixed_nodes, 2 * fixed_nodes + 1])
            free_dofs = np.setdiff1d(np.arange(self.ndof_mech), fixed_dofs)
    ...
    def optimize(self, max_iter=50):
        ...
        for k in range(max_iter):
            ...
            volconst = current_vol / (self.volfrac * self.num_elems) - 1
            fval = volconst  # Column vector of size (1xm)
            dfdx = np.ones((1, self.num_elems)) / (volfrac * self.num_elems)  # shape (1, n)
            ...
            inputs = MMAInputs(
                ...
                xold1=self.xold1,
                xold2=self.xold2,
                df0dx=dfdx_filt * 1e13,  # Negative because MMA minimizes, we maximize f
                fval=fval,
                dfdx=dfdx,
                low=self.low,
                upp=self.upp,
            ...
            # self.x = np.clip(self.x, 1e-2, 1.0)
            ...
            print(f"Iter: {k} | Obj: {f_val:.4e} = {MC:.4e} / {C33:.4e} | Vol: {current_vol / (self.num_elems):.3f} | Change: {change:.4f}")

            if change < 0.001 and k > 20:
...
if __name__ == "__main__":
    ...
    # 1. Define Physical Size
    # e.g., A MEMS flexure 5mm x 5mm, thickness 0.1mm
    L_physical = 0.001   # 0.1 mm
    thickness = 0.001   # 0.01 mm

    # 2. Choose Material
    my_material = POLYPROPYLENE

    # 3. Instantiate
    opt = ThermalFlexureTopologyOptimization(
        nelx, nely, volfrac, penal, rmin,
        material=my_material,
        phys_length_x=L_physical,
        thickness=thickness
    )

    # print('--------------')
    best_tf = run_thermal_calibration(opt, max_time_seconds=1000, steps=100)
    # run_thermal_analysis(opt, max_time_seconds=1000, steps=100)
    # exit(0)

    # x_solid = np.ones(opt.num_elems) * 1.0
    # opt.diagnose_magnitudes(x_solid)

    # The class now automatically calculates the correct dt based on physics
    # print(f"Time step set to: {opt.dt}s")


    # best_tf = 400
    opt.num_time_steps = 50
