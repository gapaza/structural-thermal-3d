...
class ThermalFlexureTopologyOptimization:
    ...
    def solve_physics(self, x):
        ...
        # nodes_RC = np.arange((self.nely + 1) * self.nelx, node_BR+1)
        # Pt = np.zeros(self.ndof_therm)
        # Pt[nodes_RC] = 100.0  # Watts of heat
        #
        # # Initial Condition
        ...
        node_BM2 = self.nely + int(self.nelx / 4) * (self.nely + 1)  # x=half, y=nely
        # node_BM = 0

        fixed_nodes = np.array([node_BM, node_BM2])
        ...
        node_TL = 0
        dof_TR_x = 2 * node_TL  # X-direction
    ...
    def optimize(self, max_iter=50):
        ...
        for k in range(max_iter):
            ...
            inputs = MMAInputs(
                ...
                xold1=self.xold1,
                xold2=self.xold2,
                df0dx=-dfdx_filt * 1e15,  # Negative because MMA minimizes, we maximize f
                fval=fval,
                dfdx=dfdx,
                low=self.low,
                upp=self.upp,
...
def run_thermal_analysis(optimizer, max_time_seconds=200, steps=100):
    ...
    for t in range(len(Phi_history)):
        ...
        Ft = Ft_history[t]
        Ft_u = Ft[0::2].reshape((optimizer.nelx + 1, optimizer.nely + 1))
        Ft_v = Ft[1::2].reshape((optimizer.nelx + 1, optimizer.nely + 1))
        Ft_history_u.append(Ft_u)
        Ft_history_v.append(Ft_v)

        U1 = U1_history[t]
        U1_u = U1[0::2].reshape((optimizer.nelx + 1, optimizer.nely + 1))
        U1_v = U1[1::2].reshape((optimizer.nelx + 1, optimizer.nely + 1))
...
if __name__ == "__main__":
    ...
    L_physical = 0.00001   # 10 cm
    thickness = 0.000001  # 0.1 cm
